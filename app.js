...
const EXTRA = {
  zh: { all:"全部", guide:"教學取向", value:"價值取向", fun:"搞笑取向", overall:"總排名", heat:"互動熱度", view:"播放", like:"點讚", comment:"留言", share:"分享", collect:"收藏", danmaku:"彈幕", favorite:"收藏夾", coins:"投幣", source:"原始連結", caption:"字幕檔", transcript:"ASR 校對稿", tags:"主類別與標籤", status:"播放與字幕狀態", analysis:"摘要分析", pending:"待補來源", playable:"可播放", bilibili:"Bilibili 備援", youtube:"YouTube 已就緒", selected:"已選", clear:"清空勾選", export:"匯出 Brief (.txt)", copy:"複製本支 Brief", add:"加入匯出", remove:"移出匯出", brief:"Creator Brief", localTitle:"Localized title", primaryCategory:"Primary category", why:"Why it worked", copyField:"What to copy", angle:"Optional angle for local creators", sourceField:"Source link", copied:"已複製", noSelection:"請先勾選至少 1 支影片", exportReady:"可直接轉發給當地團隊的精簡文字包", fallbackTitle:"目前此語言標題仍可能保留原始標題" },
  en: { all:"All", guide:"Guide & Tutorials", value:"Rewards & Value", fun:"Fun & Memes", overall:"Overall ranking", heat:"Engagement", view:"Views", like:"Likes", comment:"Comments", share:"Shares", collect:"Collects", danmaku:"Danmaku", favorite:"Favorites", coins:"Coins", source:"Source", caption:"Subtitle file", transcript:"ASR transcript", tags:"Primary category & tags", status:"Playback & subtitle status", analysis:"Editorial analysis", pending:"Pending source", playable:"Playable", bilibili:"Bilibili fallback", youtube:"YouTube ready", selected:"Selected", clear:"Clear picks", export:"Export Brief (.txt)", copy:"Copy this brief", add:"Add to export", remove:"Remove from export", brief:"Creator Brief", localTitle:"Localized title", primaryCategory:"Primary category", why:"Why it worked", copyField:"What to copy", angle:"Optional angle for local creators", sourceField:"Source link", copied:"Copied", noSelection:"Select at least one video first", exportReady:"A lean text pack that can be forwarded directly to local teams", fallbackTitle:"Some titles may still fall back to the original source title" },
};
...
function localizedTitle(video){
  return (video.localized_titles && (video.localized_titles[state.lang] || video.localized_titles.zh))
    || (video.copy && video.copy[state.lang] && video.copy[state.lang].title)
...
function localizedAnalysis(video){ return (video.localized_editorial_analysis && (video.localized_editorial_analysis[state.lang] || video.localized_editorial_analysis.zh)) || {}; }
...
function renderFilters(){ $("campaignFilters").innerHTML = ["all","guide","value","fun"].map(key => `<button class="filterBtn ${state.filter===key?'active':''}" onclick="selectFilter('${key}')">${ex(key)}</button>`).join(""); }
function renderLang(){
...
function renderList(){
  ...
  $("list").innerHTML = visible.map(v => {
    ...
    const statusBadge = v.youtube_video_id ? `<span class="badge play">${ex('youtube')}</span>` : (v.platform === 'bilibili' ? `<span class="badge play">${ex('bilibili')}</span>` : `<span class="badge wait">${ex('pending')}</span>`);
...
function renderSourceTabs(video, active){ $("sourceTabs").innerHTML = sourceList(video).map(s => `<button class="sourceTab ${active && active.type===s.type?'active':''}" onclick="selectSource('${s.type}')">${s.label}</button>`).join(""); }
...
function loadYouTube(videoId){
  ...
  if (!ytPlayer) {
    ytPlayer = new YT.Player("ytPlayer", { videoId, playerVars: { rel:0, modestbranding:1, playsinline:1, cc_load_policy:0 }, events: { onReady: () => updateSubtitle() } });
  } else if (ytPlayer.loadVideoById) ytPlayer.loadVideoById(videoId);
}
function parseTimecode(raw){ const m = raw.trim().match(/(\d+):(\d+):(\d+)[,.](\d+)/); return m ? Number(m[1])*3600 + Number(m[2])*60 + Number(m[3]) + Number((m[4]+"000").slice(0,3))/1000 : 0; }
...
function updateSubtitle(){
  ...
  if (!subtitleEl || !ytPlayer || !ytPlayer.getCurrentTime || !state.captions.length) { if (subtitleEl) subtitleEl.textContent = ""; return; }
  let current = 0;
...
function statItems(video){
  ...
  if (video.platform === 'bilibili') return [[ex('view'), s.view], [ex('like'), s.like], [ex('comment'), s.reply], [ex('share'), s.share], [ex('favorite'), s.favorite]];
...
function renderPlayer(video, source){
  if (!source) { renderPlaceholder('No playable source yet.'); return; }
  if (source.type === 'youtube' && video.youtube_video_id) { ensurePlayerHost(); loadYouTube(video.youtube_video_id); return; }
  if (source.type === 'bilibili' && source.embed_url) { ytPlayer = null; $("playerShell").innerHTML = `<iframe src="${source.embed_url}" allowfullscreen></iframe><div class="subtitleOverlay"><div class="subtitleText" id="subtitleText"></div></div>`; return; }
...
function briefFields(video){
  const analysis = localizedAnalysis(video);
  return {
    title: localizedTitle(video),
    category: ex(video.primary_campaign_category),
    why: analysis.why || video.why_it_performed_zh || '',
    copy: analysis.replicable_approach || video.replicable_approach_zh || '',
    angle: analysis.content_focus || video.content_focus_zh || '',
    source: video.source_url || ''
  };
}
function composeBrief(video){
  const fields = briefFields(video);
  return [
    `${ex('localTitle')}: ${fields.title}`,
    `${ex('primaryCategory')}: ${fields.category}`,
    `${ex('why')}: ${fields.why}`,
    `${ex('copyField')}: ${fields.copy}`,
    `${ex('angle')}: ${fields.angle}`,
    `${ex('sourceField')}: ${fields.source}`,
  ].join("\n");
}
...
$("exportSelectedBtn").addEventListener("click", () => {
  ...
  const text = composeBatchBrief(videos);
  downloadTextFile(text, `fortune-run-brief-${currentLangCode()}-${videos.length}.txt`);
});
